<template>
    <div>
        <el-form size="mini" :model="changeOrder" :rules="rules" ref="changeOrderForm" label-width="120px">
            <el-card shadow="hover">
                ...
                <div>
                    <el-row :gutter="20">
                        ...
                        <el-col :span="8" v-show="!changeOrder.id">
                            <el-form-item label="单号录入方式:">
                                <el-select size="mini" v-model="changeOrder.codeGeneratorType">
                                    <el-option key="AUTO" label="自动生成" value="AUTO"></el-option>
                                    <el-option key="SELF" label="手动录入" value="SELF"></el-option>
                                </el-select>
                            </el-form-item>
                        </el-col>
                        <el-col :span="8" v-show="changeOrder.id||changeOrder.codeGeneratorType=='SELF'">
                            <el-form-item label="单号:" prop="code" :required="changeOrder.codeGeneratorType=='SELF'">
                                <el-input :disabled="isEdit" size="mini" v-model="changeOrder.code"></el-input>
                            </el-form-item>
                        </el-col>
                    </el-row>
                    <el-row :gutter="20">
                        <el-col :span="16">
                            <el-form-item label="客户名称:" prop="customer.name">
                                <el-input size="mini"  class="input-with-select" v-model="changeOrder.customer.name"  disabled  style="float:left;width:400px">
                    ...
                    <el-row :gutter="20">
                        <el-col :span="8">
                            <el-form-item label="原因:" prop="reason">
                                <el-select size="mini" style="float: left;width: 400px"  v-model="changeOrder.reason">
                    ...
                    <el-row :gutter="20">
                        <el-col :span="8">
                            <el-form-item label="问题描述:" prop="problemDescription">
                                <el-input type="textarea" v-model="changeOrder.problemDescription"></el-input>
                            </el-form-item>
                        </el-col>
                    </el-row>
                </div>
            ...
            <el-card shadow="hover">
                <el-form-item>
                    <el-button @click="saveChangeOrder" type="primary">保存</el-button>
        ...
        <el-dialog :visible.sync="detailDialogVisible" :title="detailDialogTitle" :close-on-click-modal="false" :append-to-body="true">
            <change-order-detail-form :oldDetail="oldDetail" @close="closeDetailDialog" @callback="detailCallback"></change-order-detail-form>
        </el-dialog>
    </div>
</template>
...
<script>
    ...
    import ChangeOrderDetailForm from "@/views/erp/aftersales/changeOrder/ChangeOrderDetailForm";

    export default {
        name: "ChangeOrderForm",
        components:{ChangeOrderDetailForm, CustomerDialog},
        props: {
            isEdit: {
                type: Boolean,
                default: false
        ...
        watch: {
            oldChangeOrder: {
                handler(val) {
                    this.changeOrder = JSON.parse(JSON.stringify(val));
                },
                deep:true,
                immediate:true
            }
        ...
        methods: {
            ...
            detailCallback(detail) {
                if(this.detailIsEdit){
                    if(this.isEdit){
                        this.putNoEnCodeRequest('/erp/changeOrder/detail/update',detail).then((resp)=>{
                            if(resp&&resp.data.status=="200"){
                                this.$message.success("更新成功");
                                this.changeOrder.details.some((item,i)=>{
                                    if(item==this.oldDetail){
                                        this.changeOrder.details.splice(i,1,detail);
                                    }
                                })
                                this.detailDialogVisible = false;
                            }else{
                    ...
                    if(this.isEdit){
                        Object.assign(detail,{changeOrderId:this.changeOrder.id});
                        this.postNoEnCodeRequest('/erp/changeOrder/detail/add',detail).then((resp)=>{
                            if(resp&&resp.data.status==200){
                                this.$message.success("保存成功");
                                this.changeOrder.details.push(resp.data.detail);
                                this.detailDialogVisible = false;
                            }else{
                        ...
                        this.changeOrder.details.push(detail);
                        this.detailDialogVisible = false;
                    }
            ...
            deleteDetail(row) {
                if(this.isEdit){
                    this.$confirm("确定要删除吗?","提示",{
                        ...
                        this.deleteRequest('/erp/changeOrder/detail/'+row.id).then((resp)=>{
                            if(resp&&resp.data.status=="200"){
                                this.$message.success("删除成功");
                                this.changeOrder.details.some((item,i)=>{
                                    if(item==row){
                                        this.changeOrder.details.splice(i,1);
                                        return true;
                                    }
                                })
                            }else{
            ...
            saveChangeOrder() {
                this.$refs['changeOrderForm'].validate((valid)=>{
                    if(valid){
                        let length = this.changeOrder.details.length;
                        if(length>0){
                            if(this.isEdit){
                                this.putNoEnCodeRequest('/erp/changeOrder/updateChangeOrder',this.changeOrder).then((resp)=>{
                                ...
                                this.postNoEnCodeRequest('/erp/changeOrder/addChangeOrder',this.changeOrder).then((resp)=>{
                                    if(resp.data.status==="200"){
                                        this.$message.success("保存成功");
                                        this.$emit('callback');
                                    }else{
