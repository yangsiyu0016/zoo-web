...
<script>
    import PurchaseTaskDetails from "@/views/oa/task/PurchaseTaskDetails";
    export default {
        name: "PurchaseTask",
        components: {PurchaseTaskDetails},
        mounted(){
            this.loadTask();
        },
        methods:{
            ...
            //显示任务办理页面
            showDetailView(row){
                this.getRequest('/flow/task/getPurchaseTaskById?taskId='+row.id).then((resp)=>{
            ...
            //加载任务列表
            loadTask(){
                this.getRequest('/flow/task/getPurchaseTask?page='+this.currentPage+'&size=10').then((resp)=>{
