<template>
    <div>
        <el-container>
            ...
            <el-main>
                <el-table @sort-change="sortChange" :data="tasks" size="mini" v-loading="loading" @row-dblclick="showDetailView">
                    <el-table-column
                            type="index"
                            align="left"
                            width="80">
...
<script>
    import ProductSplitTaskDetails from "@/views/oa/task/productsplit/ProductSplitTaskDetails";
    export default {
        name: "ProductSplitTask",
        components: {ProductSplitTaskDetails},
        mounted() {
            this.loadTask();
        },
        methods: {
            ...
            showDetailView(row) {
                this.getRequest('/flow/task/getProductSplitTaskById?taskId=' + row.id).then(resp => {
                    if (resp && resp.status == 200) {
                        ...
                        if (this.currentTask.taskKey === 'productsplitckzg' && row.assigneeName != null) {
                            this.rejectVisible = true;
                        }else {
                            this.rejectVisible = false;
                        }
                        if (this.currentTask.taskKey === 'productsplitreject') {
                            this.canEdit = true;
                        }else  {
                            this.canEdit = false;
                        }
            ...
            loadTask() {
                this.loading = true;
                this.getRequest('/flow/task/getProductSplitTask?page=' + this.currentPage + '&size='+this.pageSize+'&sort='+this.sort+
                '&order='+this.order+
